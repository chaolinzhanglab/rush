...
KEY = "TGAAAC" #identifies sequence before guide to determine guide position
...
def count_spacers(input_file, fastq_file, output_file, guide_g): 
	...
	try:
		with open(input_file, mode='r', encoding = 'utf-8-sig') as infile: #rU mode is necessary for excel!  
	...
	for record in readiter: #contains the seq and Qscore etc.
		...
		if key_index >= 0:
			...
			guide = read_sequence[start_index:(start_index + 25)]
			guide1 = read_sequence[start_index:(start_index + 22)]
			if guide in dictionary:
				dictionary[guide] += 1
				perfect_matches += 1
			elif guide1 in dictionary: 
				dictionary[guide1] += 1
				perfect_matches += 1
			else:
